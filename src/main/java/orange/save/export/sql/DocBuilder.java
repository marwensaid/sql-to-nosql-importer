...
/**
 * <pattern> {@link DocBuilder} is responsible for creating Solr documents out of the given configuration. It also maintains
 * statistics information. It depends on the {@link EntityProcessor} implementations to fetch data. </pattern>
 * <pattern/>
...
public class DocBuilder {
    ...
    public void execute() throws Exception {
        ...
        String url = dataImporter.getConfig().dataSources.get(null).getProperty("url");
        String user = dataImporter.getConfig().dataSources.get(null).getProperty("user");
        String password = dataImporter.getConfig().dataSources.get(null).getProperty("password");
        batchSize = Integer.valueOf(dataImporter.getConfig().dataSources.get(null).getProperty("batch-size"));
        Entity rootEntity = dataImporter.getConfig().document.entities.get(0);
        String driverName = dataImporter.getConfig().dataSources.get(null).getProperty(DRIVER);
        ...
        dataImporter.getWriter().setPrimaryKey(rootEntity.pk);

        if (rootEntity.isDocRoot) {
            ...
            while (rs.next()) {
                if (i == dataImporter.getAutoCommitSize()) {
                    long t2 = System.currentTimeMillis();
                    log.info("Time taken to Read " + i + " documents from SQL : "
                            + (t2 - t1) + " ms");
                    dataImporter.getWriter().writeToNoSQL(entityList);
    ...
    public Map<String, Object> getFields(Map<String, Object> firstRow, ResultSet resultSet, Entity entity, Map<String, Object> entityMap, Map<String, Object> rootEntityMap) throws SQLException {
        ...
        if (entity.entities != null) {
            ...
            for (Iterator<Entity> iterator = entity.entities.iterator(); iterator.hasNext(); ) {
                ...
                if (this.resultSet.next()) {
                    subEntityData = getFields(processor.toMap(this.resultSet), this.resultSet, subEntity, null, entityMap);
    ...
    private void getMultiValuedEntity(ResultSet resultSet, Entity entity, Map<String, Object> rootEntityMap) throws SQLException {
        ...
        while (resultSet.next()) {
            if (entity.fields.size() > 1) {
                ...
                for (Iterator<Field> iterator = entity.fields.iterator(); iterator.hasNext(); ) {
                    ...
                    entityFieldsMap.put(field.name, convertFieldType(fieldType, resultSet.getObject(field.column)).get(0));
                }
    ...
    private void getSingleValuedEntity(Map<String, Object> firstRow, ResultSet resultSet, Entity entity, Map<String, Object> entityMap) throws SQLException {
        int totalRows = resultSet.getMetaData().getColumnCount();

        for (int i = 0; i < totalRows; i++) {
            extractFieldValue(firstRow, entity, entityMap, resultSet.getMetaData().getColumnLabel(i + 1),
                    resultSet.getMetaData().getColumnLabel(i + 1), null);
        }
    ...
    private void extractFieldValue(Map<String, Object> firstRow, Entity entity, Map<String, Object> entityMap, String fieldName, String fieldColumn, FieldType fieldType) {
        if (entity.pk != null && entity.pk.equals(fieldName)) {
            if (dataImporter.getDataStoreType().equals(DataStoreType.MONGO) || dataImporter.getDataStoreType().equals(DataStoreType.COUCH)) {
